...
```{r}
list.of.packages <- c("ggplot2", "broom", "MASS", "rmarkdown", "knitr")
...
```{r echo=FALSE}
knitr::kable(tidy(lmfit), format = "html")
```
...
```{r echo=FALSE}
knitr::kable(glance(lmfit), format = "html")
```
...
knitr::kable(head(augment(lmfit)), format = "html")
```
...
```{r echo=FALSE}
knitr::kable(tidy(t.test(diamonds$price,
            conf.level = 0.95)), format = "html")
```
...
```{r echo=FALSE}
knitr::kable(tidy(prop.test(with(diamonds,table(cut!="Ideal")))), format = "html")
```
...
```{r echo=FALSE}
knitr::kable(tidy(prop.test(with(diamonds,table(cut!="Ideal")),
                 conf.level = 0.99)), format = "html")
```
...
PrixDiff <- t.test(diamonds$price, 
            ...
            var.equal = FALSE, 
            conf.level = 0.95)
knitr::kable(tidy(PrixDiff), format = "html")
```
...
PrixPlusGrand <- t.test(diamonds$price, 
                  ...
                  var.equal = FALSE, 
                  conf.level = 0.90)
knitr::kable(tidy(PrixPlusGrand), format = "html")
```
...
IdealPlusPetit <- prop.test(with(diamonds,table(cut!="Ideal")),
                    ...
                    alternative = "less",
                    conf.level = 0.95)
knitr::kable(tidy(IdealPlusPetit), format = "html")
```
...
IdealPremiumDiff <- t.test(formula = price ~ cut,
                      ...
                      var.equal = FALSE,
                      conf.level = 0.99)
knitr::kable(tidy(IdealPremiumDiff), format = "html")
```
...
BarleyPaired <- t.test(immer$Y1, 
                       immer$Y2,
                       paired=TRUE)
knitr::kable(tidy(BarleyPaired), format = "html")
```
...
```{r echo=FALSE}
PropPremiumE <- prop.test(with(diamonds,table(cut == "Premium",color == "E")))
knitr::kable(tidy(PropPremiumE), format = "html")
```
...
```{r echo=FALSE}
ChiCut <- chisq.test(x = table(diamonds$cut))
knitr::kable(tidy(ChiCut), format = "html")
```
...
```{r echo=FALSE}
ChiCutColor <- chisq.test(x = table(diamonds$cut, diamonds$color))
knitr::kable(tidy(ChiCutColor), format = "html")
```